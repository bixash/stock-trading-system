...
export function EditOrderDialog({
  ...
  useEffect(() => {
    async function fetchOrderWithId(orderId: number) {
      try {
        const response = await api.get(`/orders/${orderId}`);
        setOrder(response.data.result);
        setType(order?.transactionType);
        setSymbol(order?.ticker);
      } catch (error) {}
    }
    fetchOrderWithId(orderId);
  }, []);
  ...
  }, [symbol, type, order, currentUser, form]);
  ...
  const onSubmit = async (orderData: formSchema) => {
    try {
      ...
      const response = await api.put(`/orders?orderId=${orderId}`, orderData);
      if (response.data) {
      ...
      <Form {...form}>
        ...
        >
          <DialogContent className="w-[600px]">
            ...
            >
              <div>
                <DialogTitle className="text-[15px]">{symbol}</DialogTitle>
            ...
            <DialogFooter className="-mt-4 py-3 px-6 bg-bg_0">
              <div className="flex gap-2 items-center">
                ...
                <span className=" inline-block text-sm text-bg_2">
                  Charges <span className="text-bg_3"> {amount * 0.02}</span>
                </span>
              </div>
